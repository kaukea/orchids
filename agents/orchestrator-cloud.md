...
3. **Write the board handoff.** Flip the task's status to `doing`,
   delegated-to `architect-cloud`, on `main`, and commit that change.
   **Board writes are your exclusive right** — no other cloud role
   (architect-cloud, housekeeper-cloud) ever touches `docs/TODO.md`.
